...
static inline char rand_char(void) { return (char)((rand() % 255) + 1); }
...
static __attribute_used__ TrieNode* gen_singleton_wlen(test_result_t* res,
						       size_t keylen)
{
...
TEST_DEFINE(test_delete, res)
{
	...
	for (size_t iters = 0; iters < N; ++iters) {
		...
		trie_delete(trie_b, "");
		compact = compact && test_compact(trie_b);
		empty_noaffect = empty_noaffect && trie_a->root &&
				 trie_a->root->segment[0] == '\0' &&
				 trie_b->root &&
				 trie_b->root->segment[0] == '\0';

		add_delete =
		    add_delete && tries_equal(trie_a->root, trie_b->root);
		trie_destroy(trie_a);
		trie_destroy(trie_b);
	}
...
TEST_DEFINE(test_key_add_segment, res)
{
	...
	bool last_concat = strcmp(oldbuf, "Hello world!Add") == 0 &&
...
TEST_DEFINE(test_iterator, res)
{
	...
	size_t N = rand() & 1 ? 10 : 100, n_kv = gen_len_bw(100, 200),
	       pf_len = rand() & 1 ? 0 : 3,
	       max_keylen = gen_len_bw(N / 4, 3 * N / 4), n_findable = 0,
	       n_found = 0;
	key_val_t* kv = malloc(n_kv * sizeof kv[0]);
	char* prf = NULL;
	for (size_t i = 0; i < n_kv; ++i) {
...
TEST_START(test_instantiation, asan_test_destroy, test_max_keylen,
	   test_node_create, test_insert, test_delete, test_find, test_segncpy,
	   test_key_add_segment, asan_test_iter_destroy, test_iterator, )
